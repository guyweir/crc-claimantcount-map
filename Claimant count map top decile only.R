...
cc <- filter(cc,`Change decile (1 = low)`==10)
...
table <- cc  %>% select(-`Change decile (1 = low)`)
...
tbl <- reactable(table2, selection = "multiple",
                 ...
                 columns = list(`Local Authority` = colDef(filterable = T),
...
EWS.centroids.df$ccradius <- (EWS.centroids.df$`Claimant count rate ppt change (August 2017 - August 2020)`)* 0.5 
...
addLegendCustom <- function(map, colors, labels, sizes, opacity = 0.8, position){
...
#map element
m2 <- leaflet(EWS.centroids.dfXT, height = "680px", options = list(padding = 100)) %>% setView(-3.5,54.2, 6) %>% 
  ...
  addCircleMarkers( group = "circlegw",
                    radius = ~`ccradius`,
                    stroke = F,
                    color = ~factpal(`Change decile (1 = low)`), opacity = 0.85, fillOpacity = 0.4,
                    label = labels,
  ...
  addLegendCustom(colors = c("#660C21","#660C21","#660C21" ), 
                  labels = c("6ppts", "10ppts","15ppts"),
                  
                  sizes = c(3, 5, 8)*2, position = "bottomright" ) %>% 
